...
class SwarmControlNode(Node):
    ...
    def position_callback(self, msg: PointStamped):
        ...
        self.positions[drone_id] = np.array([
            msg.point.x,
            msg.point.y,
            msg.point.z
        ])
    ...
    def compute_graph(self):
        ...
        for i, id_i in enumerate(ids):
            for j, id_j in enumerate(ids):
                if i != j:
                    graph[i, j] = np.linalg.norm(self.positions[id_i] - self.positions[id_j])
        return graph, ids
    ...
    def control_loop(self):
        ...
        if self.state == "IN_AIR":
            if all(state == "IN_AIR" for state in self.states.values() if state is not None) and len(self.states) == len(self.positions):
                self.state = "SWARMING"
            return
        ...
        if self.state == "SWARMING":
            ...
            for j, other_id in enumerate(self.id_list):
                ...
                velocity_magnitude = ((distance - spring_length) / self.spring_constant) ** 2
